...
<div class="container">
    ...
    <section class="content-header">
        ...
        <ol class="breadcrumb" style="font-size:20pt;">
            <li><i class="fa fa-dashboard"></i> <a href="/sakade/">工事一覧へ</a></li>
        </ol>
    </section>
    <hr/>
                                ...
                                <div class="col-md-8" id="image-box-base">
                                    ...
                                    <div  id="koujiImg">
                                        <%--<c:if test="${not empty firstReportImage}">--%>
                                            <%--<img  id="previewImage" class="previewImage"  style="border:solid 1px silver" width="60%"  src="http://storage.googleapis.com/sakade/${firstReportImage.imagename}" width="600" />--%>
                                                        ...
                                                        <%--<div id="${image.imagename}" class="col-md-2 image-div" >--%>
                                                            <%--<div class="row thumbnail-object">--%>
                                                                <%--<div class="col-md-12 thumbnail-img">--%>
                                                                    <%--<img src="http://storage.googleapis.com/sakade/${image.imagename}" onclick="changeImage(this)" alt="..." style="cursor:pointer;height: 100px">--%>
                                                                <%--</div>--%>
                                                            <%--</div>--%>
                                                        <%--</div>--%>
                                                    <%--</c:forEach>--%>
                ...
                <div class="row">
                    <div class="col-md-12">
                        <ul class="nav nav-pills nav-stacked bookmarkUl">
                            ...
                            <li role="presentation" class="kengen-operation"><a href="/sakade/kouji/${kouji.id}/instruct"><i class="glyphicon glyphicon-indent-left"> 指示</i></a></li>
                            ...
                            <li role="presentation" class="currentBookmark"><a href="/sakade/kouji/${kouji.id}/image"><i class="glyphicon glyphicon-picture"> 図面</i></a></li>
...
//        imagename=imagename.replace("http://storage.googleapis.com/sakade/","");

//        $.post("/sakade/image/getReportimageKikisystemByKoujiAndImagename",
//                {"koujiId":koujiId,
//                    "imagename":imagename},
//                function(data){
...
//                console.log("tmp="+tmp+"   ;style ="+style );
    ...
    function submitKanrenben(){
        ...
        $.post("/sakade/image/updateImageKikisystem",{"koujiId":koujiId,"imagename":imagename,"selectedIdList":selectedIdList},function(data){
            if(data=="login"){
//                logoutFun();
                console.log("logout updateImageKikisystem 111");
            }
            $.post("/sakade/image/getReportimageKikisystemByKoujiAndImagename",
                    {"koujiId":koujiId,
                        "imagename":imagename},
                    function(data){
                        if(data=="login"){
//                            logoutFun();
                            console.log("logout getReportimageKikisystemByKoujiAndImagename 222");
                        }
                        var items = JSON.parse(data);
                        //current-select-ben
                        var htmlContent = "";
            ...
            $("#myModal").modal("hide");
//            $.post("/sakade/image/saveImageKikisystem",{"koujiId":koujiId,"imagename":imagename,"selectedIdList":selectedIdList},function(data){});
        });
    }
    ...
    function removeImage(){
        ...
        imagename=imagename.replace("http://storage.googleapis.com/"+BUCKET+"/","");
        //delete session
        $.post("/sakade/image/deleteByImagename",
                {"object":imagename},
                function(data){
                    if(data=="login"){
                        logoutFun();
                    ...
                    var koujiId = document.getElementById("koujiId").value;
                    $.post("/sakade/image/deleteDatabaseByImagename",
    ...
    function updatePage(imagename){
        $.post("/sakade/image/updatePageNumber",{"imagename":imagename},function(data){
            if(data=="login"){
                logoutFun();
            }
            var items = JSON.parse(data);
    ...
    function turnToSet(setNumStr){
        ...
        if(setNum >= 1 && setNum <= totalSet){
            setNum = setNum - 1;
            $.post("/sakade/image/getReportimageBySet", {"setNum": setNum}, function (data) {
                if(data=="login"){
                    logoutFun();
                }
                var items = JSON.parse(data);
...
//                        $(obj).removeAttr("disabled");
            ...
            //update database
            $.post("/sakade/image/saveSyuByImagename",
                    {"imagesyu": $(obj).val(), "object": imagename},
                    function (data) {
                        if(data=="login"){
                            logoutFun();
...
//        var currentObject = link.split("http://storage.googleapis.com/sakade/")[1];
//        object = currentObject;
//        document.getElementById("currentImageObject").value = currentObject;
//        $.post("/sakade/image/getImageByImagename",{"imagename":currentObject},function(data){
...
//    }
    ...
    function submitBikou(){
        if(object.length > 0){
            ...
            $.post("/sakade/image/submitBikouById",{"bikou":$("#buhinzubikou").val(),"object":object},function(data){
                if(data=="login"){
                    logoutFun();
                }
    ...
    $(window).unload(function(){
        $.post("/sakade/image/saveSessionToDatabase");
    });
    ...
    function changeNumPage(){
        ...
        if(setImageNumNew>"${totalSet}" || setImageNumNew<1){
            ...
            $.post("/sakade/image/changeImagePage",{"koujiId":koujiId,"imagename":imagename,"pageNew":setImageNumNew,"pageOld":setImageNumOld},function(data){
...
function executeRequestInsert(objectName) {
        ...
        //update session
        var koujiId = document.getElementById("koujiId").value;
        $.post("/sakade/image/updateImageByImagePath",{"object":objectName,"koujiId":koujiId},function(data){
            if(data=="login"){
                logoutFun();
            }
            progressBarController(0);
        ...
        //update database
        $.post("/sakade/image/saveImageByImagePath",{"object":objectName,"koujiId":koujiId},function(data){
            if(data=="login"){
                logoutFun();
            }
            //図面件数を更新
