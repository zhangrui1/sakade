        ...
        <div class="row">
            ...
            <div class="col-md-4">
                <div class="btn-group" role="group">
                    <a href="/sakade/list/valveMult" class="btn btn-default bg-yellow active" onclick="return saveSelectedForValve()">複合検索</a>
                    <%--<a href="/sakade/list/valve" class="btn btn-default" onclick="return saveSelectedForValve()">弁検索</a>--%>
        ...
        <div class="row">
            <div class="panel panel-success">
                ...
                <div class="panel-body" id="searchForValve" style="display:block;">
                    <div class="row">
                        <div class="col-md-14">
                            <form action="/sakade/item/valveMultSearchForSeikak" method="post" class="sidebar-form"  name="searchForValve" onsubmit="return checkValveSearchForSeikak();">
                                <div class="box-body">
                                        <div class="row">
                                            <div class="col-md-12">
                                        ...
                                        <div class="row">
                                            <div class="col-md-1">
                                                <span STYLE="color: red;">設置プラント</span>
                                            </div>
                                            <div class="col-md-5" >
                                                <div class="input-group">
                                                ...
                                                <span STYLE="color: red;">※「全部会社名」の場合、項目のいずれを入力してください</span>
                                            </div>
                                        </div><br>
                                        <div class="row">
                                            <div class="col-md-1">
                                    ...
                                    <div class="box-body clearfix">
                                        <div class="form-group">
                                            ...
                                            <div class="col-md-3">
                                                <span id="valve_num" class="valve_num pull-right" STYLE="font-size: 16px;">ヒット弁 ${valveMultSearchitemNum} 台 (${searchValveMultiTotalTime} 秒)</span>
                                                <br><span class="valve_num pull-right" STYLE="color: red;">※500件超える場合、500件まで表示する</span>
                                            </div>
                                            <div class="col-md-2">
...
//         var tr = document.getElementById("select-"+obj.value);
    ...
    function checkValveSearchForSeikak(){
        //全部会社名の場合、項目のいずれか必要です
        var VNo=$("#VNo").val();
        var BenMeisyo=$("#BenMeisyo").val();
        var SetSetubi=$("#SetSetubi").val();
        var SetKiki=$("#SetKiki").val();
        var SetBasyo=$("#SetBasyo").val();
        var ics=$("#ics").val();
        var ryutaiRyaku=$("#ryutaiRyaku").val();
        var ryutai=$("#ryutai").val();
        var keisikiRyaku=$("#keisikiRyaku").val();
        var keisiki=$("#keisiki").val();
        var sousaRyaku=$("#sousaRyaku").val();
        var sousa=$("#sousa").val();
        var classType=$("#classType").val();
        var yobikeiRyaku=$("#yobikeiRyaku").val();
        var yobikei=$("#yobikei").val();
        var szHouRyaku=$("#szHouRyaku").val();
        var szHou=$("#szHou").val();
        var szKikaku=$("#szKikaku").val();
        var zaisituRyaku=$("#zaisituRyaku").val();
        var zaisitu=$("#zaisitu").val();
        var katasikiNo=$("#katasikiNo").val();
        var makerRyaku=$("#makerRyaku").val();
        var maker=$("#maker").val();
        var syukan=$("#syukan").val();
        var buhinMei=$("#buhinMei").val();
        var hyojunSiyou=$("#hyojunSiyou").val();
        var sunpou=$("#sunpou").val();
        var buhinMakerRyaku=$("#buhinMakerRyaku").val();
        var buhinMaker=$("#buhinMaker").val();

        if(($("#locationName").val())=="全部会社名" && (checkNull(VNo)) && (checkNull(BenMeisyo))  && (checkNull(SetSetubi))
                && (checkNull(SetKiki))  && (checkNull(SetBasyo))  && (checkNull(ics))  && (checkNull(ryutaiRyaku))
                && (checkNull(ryutai))  && (checkNull(keisikiRyaku))  && (checkNull(keisiki))  && (checkNull(sousaRyaku))
                && (checkNull(sousa))   && (checkNull(classType))  && (checkNull(yobikeiRyaku))
                && (checkNull(yobikei))  && (checkNull(szHouRyaku))  && (checkNull(szHou))  && (checkNull(szKikaku))
                && (checkNull(zaisituRyaku))  && (checkNull(zaisitu))  && (checkNull(katasikiNo))  && (checkNull(makerRyaku))
                && (checkNull(maker))  && (checkNull(syukan))  && (checkNull(buhinMei))  && (checkNull(hyojunSiyou))
                && (checkNull(sunpou))  && (checkNull(buhinMakerRyaku))  && (checkNull(buhinMaker))){
            console.log("false");
            alert("設置プラントは「全部会社名」の場合、項目のいずれを入力してください");
            return false;
        }else{
            console.log("true");
            return true;
...
//                console.log("no null =;"+code+";");
     ...
     //選択された弁を記録する
        function saveSelectedForValve(){
            ...
            //Filter sessionに保存
            $.get("/sakade/item/saveSelectedForValve",{"idList":idList,"type":"4"},function(data){
                if(data=="login"){
                    logoutFun();
                }
                return true;
