...
<link type="text/css" rel="stylesheet" href="/sakade/css/image.css"/>
<html>
<style type="text/css">
...
<div class="row">
    ...
    <div class="col-md-6">
        <div class="btn-group" role="group">
            <a href="/sakade/item/${valve.kikiSysId}/${kikiOrBenFlg}/valve" class="btn btn-default">弁基本情報</a>
            <a href="/sakade/kenan/kenan/${valve.kikiSysId}/${kikiOrBenFlg}/valve" class="btn btn-default">懸案</a>
            <a href="/sakade/tenken/tenken/${valve.kikiSysId}/${kikiOrBenFlg}/valve" class="btn btn-default">点検履歴</a>
            <a href="/sakade/image/image/${valve.kikiSysId}/${kikiOrBenFlg}/valve" class="btn btn-default  bg-yellow active">図面</a>
            <a href="/sakade/list/valveMult/" class="btn btn-default">戻る</a>
        </div>
        <div class="pull-right">
            (${searchValveImageTotalTime} 秒)
        </div>
...
<div class="box box-solid box-image" id="div-1" style="display: block">
    <div class="box-header box-panel">
        <h3 class="box-title"><i class="fa fa-fw fa-files-o"></i> ${valve.vNo}の ICS図面 ${IcsImageListSize}枚</h3>
    </div>
...
<div class="box box-solid box-image" id="div-2" style="display: block">
    <div class="box-header box-panel">
        <h3 class="box-title"><i class="fa fa-fw fa-files-o"></i> ${valve.vNo}の GP図面 ${GPImageListSize}枚</h3>
    </div>
    <div class="box-body" style="display: none">
        <div class="form-group">
            <%--印刷ボタンなど--%>
...
<div class="box box-solid box-image" id="div-4" style="display: block">
    ...
    <div class="box-body" style="display:none">
        <div class="form-group">
            <div class="row col-md-12">
                <div class="btn-group" role="group">
                    <button type="button"  id="typeImageKouji0" class="btn btn-default" onclick="turnToImagesyuKouji(0)"/>全部</button>
                    <button type="button"  id="typeImageKouji1" class="btn btn-default" onclick="turnToImagesyuKouji(1)"/>図面仕様書</button>
                    <button type="button"  id="typeImageKouji2" class="btn btn-default" onclick="turnToImagesyuKouji(2)"/>点検報告書</button>
                    <button type="button"  id="typeImageKouji3" class="btn btn-default" onclick="turnToImagesyuKouji(3)"/>懸案事項一覧</button>
                    <button type="button"  id="typeImageKouji4" class="btn btn-default" onclick="turnToImagesyuKouji(4)"/>完成図面</button>
                    <button type="button"  id="typeImageKouji5" class="btn btn-default" onclick="turnToImagesyuKouji(5)"/>作業指示書</button>
                    <button type="button"  id="typeImageKouji6" class="btn btn-default" onclick="turnToImagesyuKouji(6)"/>弁棒ねじ検索記録</button>
                </div>
            </div>
        </div></br></br>
        <div class="form-group">
...
var BUCKETGP = 'valdac-test'; //テストサーバ用
//var BUCKETGP = 'valdac-asia';   //本番

$(window).load(function () {
    //ICS図面がある場合取得
     if("${IcsImageListSize}" > 0){
        //最初の図面を表示する
        loadImg("${IcsFirstImage.imagename}","previewImageIcs");
     }
    //GP図面がある場合取得
    if("${GPImageListSize}" > 0){
        //最初の図面を表示する
        loadImg("${GPFirstImage.imagename}","previewImageGP");
    }
    //弁図面がある場合取得
    if("${imageListKikisysSize}" > 0){
        //最初の図面を表示する
    ...
    document.getElementById("typeImageKouji0").style.background="#eeeeff";
});
...
function setImageForKouji(items,flg) {
    if(flg){
//        document.getElementById('previewImageKouji').src="http://storage.googleapis.com/sakade/"+items.imagename;
...
function turnToImagesyuKouji(type){
    ...
    if(type=="1"){
        ...
        if(numallKikisysImageList01>0){
            //該種類のデータを設定
            $.post("/sakade/image/setReportImagesyuSessionForKouji", {"type": type}, function (data) {
                if(data=="login"){
                    logoutFun();
                }
                var items = JSON.parse(data);
...
function turnToSet(setNumStr,totalNumStr,type){
    ...
    if(setNum >= 1 && setNum <= valueTotal){
        ...
        $.post("/sakade/image/getimageByValveID", {"setNum": setNum,"type":type}, function (data) {
            if(data=="login"){
                logoutFun();
            }
            var items = JSON.parse(data);
...
function loadImg(imgUrl,ImageId) {
    document.getElementById(ImageId).src="/sakade/img/blackLoading.gif";
    ...
    if(totalSet>0){
        //javaにて　signUrlを取得する
        $.post("/sakade/SingUrlTest/SignTest",{"OBJECT_NAME":OBJECT_NAME,"BUCKET_NAME":BUCKET},function(data){
...
//                $("#loading").css({'display':'none'});
                //エラー画像設定
                document.getElementById(ImageId).src="/sakade/img/error.jpg";
            }
            //url-Set
