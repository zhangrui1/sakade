...
public class ImageController {
    ...
    public String saveImageByImagePath(@RequestParam("object")String object,
        ...
        //historyテーブルを更新
        historyValveService.addHistoryValve(Config.TReportImage,object+"",Config.TInsert,session,request);
        return "";
    }
...
//        }
    ...
    public String saveSyuReportImageList(@RequestParam("imagesyu")String imagesyu,
        ...
        //historyテーブルを更新
        historyValveService.addHistoryValve(Config.TReportImage,object+"",Config.TImageSyuEdit,session,request);
        return "";
    }
    /**
     * sessionの画像を削除する
    ...
    @ResponseBody
    public String deleteDatabaseByImagename(@RequestParam("object")String object,@RequestParam("koujiId")String koujiId,HttpSession session,ModelMap modelMap,HttpServletRequest request){
        //session　check
        User user=(User)session.getAttribute("user");
        if(user == null){
            modelMap.addAttribute("message",Config.TUserNull);
        ...
        //対象図面の関連弁リストを取得する
        List<Integer> imagenameToKikisysIDList=reportimageKikiSystemService.getKikisysIDListByReportimageKikiSystem(koujiId,object);
        for(int n=0;n<imagenameToKikisysIDList.size();n++){
            //図面関係表のkikiSysPage修正
            List<ReportimageKikiSystem> tmpReportimageKikiSystem=reportimageKikiSystemService.getKikisysIDListByReportimageKikiSystem(imagenameToKikisysIDList.get(n));
            for(int j=0;j<tmpReportimageKikiSystem.size();j++){
                tmpReportimageKikiSystem.get(j).setKikiSysPage(j+1);
                reportimageKikiSystemService.updateKikiSysPageByReportImage(tmpReportimageKikiSystem.get(j));
            }
        }
        ...
        //historyテーブルを更新
        historyValveService.addHistoryValve(Config.TReportImage,object+"",Config.TDelete,session,request);
        return "";
    }
    ...
    public String updateImageKikisystem(@RequestParam("koujiId")String koujiId,
        ...
        //DBに既存の該図面の関連弁IDリストを取得する
        List<Integer> idListPastedA=reportimageKikiSystemService.getKikisysIDListByReportimageKikiSystem(koujiId,imagename);
        //今回選択した弁IDリスト
        List<Integer> idListNewB = new ArrayList<Integer>();
        if(selectedIdList.length() > 0) {
            String idArray[] = selectedIdList.split(",");
            for (int i = 0; i < idArray.length; i++) {
                idListNewB.add(Integer.valueOf(idArray[i]));
            }
        }
        //A ,Bリストの共通部分⇒既存のまま　変更なし
        //Aに存在が、Bにない⇒削除対象
        List<Integer> idListADelete=new ArrayList<Integer>();
        List<Integer> idListBInsert=new ArrayList<Integer>();
        //Aにないが、Bに存在⇒追加対象
        for(int nIndexA=0;nIndexA<idListPastedA.size();nIndexA++){
            if(!idListNewB.contains(idListPastedA.get(nIndexA))){
                //Aに存在が、Bにない⇒削除対象
                idListADelete.add(idListPastedA.get(nIndexA));

                reportimageKikiSystemService.deleteByImagenameAndKikisysId(imagename,idListPastedA.get(nIndexA));
                //図面関係表のkikiSysPage修正
                List<ReportimageKikiSystem> tmpReportimageKikiSystem=reportimageKikiSystemService.getKikisysIDListByReportimageKikiSystem(idListPastedA.get(nIndexA));
                for(int j=0;j<tmpReportimageKikiSystem.size();j++){
                    tmpReportimageKikiSystem.get(j).setKikiSysPage(j+1);
                    reportimageKikiSystemService.updateKikiSysPageByReportImage(tmpReportimageKikiSystem.get(j));
                }
        ...
        //Aにないが、Bに存在⇒追加対象
        for(int nIndexB=0;nIndexB<idListNewB.size();nIndexB++){
            if(!idListPastedA.contains(idListNewB.get(nIndexB))){
                //Aにないが、Bに存在⇒追加対象
                idListBInsert.add(idListNewB.get(nIndexB));

                ReportimageKikiSystem reportimageKikiSystem = new ReportimageKikiSystem();
                reportimageKikiSystem.setKikiSysId(idListNewB.get(nIndexB));
                reportimageKikiSystem.setKoujiId(Integer.valueOf(koujiId));
                reportimageKikiSystem.setImagename(imagename);
                reportimageKikiSystem.setKikiSysPage(99999);
                ...
                //図面関係表のkikiSysPage修正
                List<ReportimageKikiSystem> tmpReportimageKikiSystem=reportimageKikiSystemService.getKikisysIDListByReportimageKikiSystem(idListNewB.get(nIndexB));
                for(int j=0;j<tmpReportimageKikiSystem.size();j++){
                    tmpReportimageKikiSystem.get(j).setKikiSysPage(j+1);
                    reportimageKikiSystemService.updateKikiSysPageByReportImage(tmpReportimageKikiSystem.get(j));
                }
        ...
        //20161206 図面page機能修正するために、関連表にの追加方法を修正した
...
//
//                //図面関係表のkikiSysPage修正
//                List<ReportimageKikiSystem> tmpReportimageKikiSystem=reportimageKikiSystemService.getKikisysIDListByReportimageKikiSystem(Integer.valueOf(idArray[i]));
//                for(int j=0;j<tmpReportimageKikiSystem.size();j++){
//                    tmpReportimageKikiSystem.get(j).setKikiSysPage(j+1);
//                    reportimageKikiSystemService.updateKikiSysPageByReportImage(tmpReportimageKikiSystem.get(j));
//                }
...
//        }
        ...
        newRKList = reportimageKikiSystemService.getListByKoujiId(koujiId);
        session.setAttribute("reportimageKikiSystemList",newRKList);

        //historyテーブルを更新
        historyValveService.addHistoryValve(Config.TReportimagekikisystem,imagename+"   ;"+selectedIdList,Config.TEdit,session,request);
        return "ok";
    }
    ...
    public String saveImageKikisystem(@RequestParam("koujiId")String koujiId,
                                        @RequestParam("imagename")String imagename,
                                        @RequestParam("selectedIdList")String selectedIdList,ModelMap modelMap,HttpSession session){
        //session　check
        User user=(User)session.getAttribute("user");
        if(user == null){
            modelMap.addAttribute("message",Config.TUserNull);
...
//        }
    ...
    public String image(@PathVariable String kikiSysId,@PathVariable String kikiOrBenFlg,
        ...
        double luceneTime=System.currentTimeMillis();
        System.out.println("luceneTime="+(luceneTime-begintime)/1000);
        ...
        double valvegetTime=System.currentTimeMillis();
        System.out.println("valvegetTime="+(valvegetTime-luceneTime)/1000);
        //kouji関連の図面
        ...
        double koujigetTime=System.currentTimeMillis();
        System.out.println("koujigetTime="+(koujigetTime-valvegetTime)/1000);
...
//            IcsFirstImage=IcsImageList.get(0);
        ...
        double imagebeginTime=System.currentTimeMillis();
        System.out.println("valveImageStartTime="+(imagebeginTime-koujigetTime)/1000);
        ...
        double imageendTime=System.currentTimeMillis();
        System.out.println("valveImageEndTime="+(imageendTime-imagebeginTime)/1000);
        //type 図面　list
        ...
        double koujiImagegetTime=System.currentTimeMillis();
        System.out.println("koujiImagegetTime="+(koujiImagegetTime-imageendTime)/1000);
        //type 図面　list  kouji
        ...
        //historyテーブルを更新
        historyValveService.addHistoryValve("",valve.getKikiSysId()+"",Config.TImageList,session,request);
        double totalTime=(System.currentTimeMillis()-begintime)/1000;
        System.out.println("searchValveImageTotalTime="+totalTime);
        session.setAttribute("searchValveImageTotalTime",totalTime);
