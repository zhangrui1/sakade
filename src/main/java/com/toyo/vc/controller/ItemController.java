...
//            System.out.println("kiki  件数="+searchResultObjectsKiki.size());
    ...
    public String getItemByKikisysid(@PathVariable String kikiSysId,@PathVariable String kikiOrBenFlg,ModelMap modelMap, HttpSession session,HttpServletRequest request) throws IOException {
        //session　check
        User user=(User)session.getAttribute("user");
        if(user == null){
            modelMap.addAttribute("message",Config.TUserNull);
        ...
        //historyテーブルを更新
        historyValveService.addHistoryValve("",valve.getKikiSysId()+"",Config.TKikisysDetail,session,request);
    ...
    public String getKikiByKikiId(@PathVariable String kikiSysId,
        ...
        //historyテーブルを更新
        historyValveService.addHistoryValve("",kiki.getKikiId()+"",Config.TKikiDetail,session,request);
...
//        System.out.println("search time="+(endtime-begintime));
    ...
    public String buhinSearch(@RequestParam("keyword")String keyword, @RequestParam("locationNameSelect")String locationNameSelect,@RequestParam("buhinRadioSelect")String buhinRadioSelect,ModelMap modelMap,HttpSession session) throws IOException, ParseException {
        ...
        double tmptime2=System.currentTimeMillis();
        ...
        double tmptime3=System.currentTimeMillis();
        ...
        double tmptime4=System.currentTimeMillis();
    ...
    @ResponseBody
    public String valveFilter(@RequestParam("filterKeyword")String filterKeyword,HttpSession session,ModelMap modelMap) throws UnsupportedEncodingException {
        //session　check
        User user=(User)session.getAttribute("user");
        if(user == null){
            modelMap.addAttribute("message",Config.TUserNull);
    ...
    @ResponseBody
    public String valveKikiFilter(@RequestParam("filterKeyword")String filterKeyword,HttpSession session,ModelMap modelMap) throws UnsupportedEncodingException {
        //session　check
        User user=(User)session.getAttribute("user");
        if(user == null){
            modelMap.addAttribute("message",Config.TUserNull);
    ...
    @ResponseBody
    public String valveKikiBuhinFilter(@RequestParam("filterKeyword")String filterKeyword,HttpSession session,ModelMap modelMap) throws UnsupportedEncodingException {
        //session　check
        User user=(User)session.getAttribute("user");
        if(user == null){
            modelMap.addAttribute("message",Config.TUserNull);
    ...
    @ResponseBody
    public String koujiFilter(@RequestParam("filterKeyword")String filterKeyword,HttpSession session,ModelMap modelMap) throws UnsupportedEncodingException {
        //session　check
        User user=(User)session.getAttribute("user");
        if(user == null){
            modelMap.addAttribute("message",Config.TUserNull);
    ...
    public String valveSearchForSeikak(@ModelAttribute("ValveForm")ValveForm valveForm, ModelMap modelMap,HttpSession session,HttpServletRequest request) throws IOException {
        double begintime=System.currentTimeMillis();
        //session　check
        User user=(User)session.getAttribute("user");
        if(user == null){
        ...
        //historyテーブルを更新
        historyValveService.addHistoryValve("",valve.getLocationName(),Config.TKikisysSearch,session,request);
        double totalTime=(System.currentTimeMillis()-begintime)/1000;
        session.setAttribute("searchValveTotalTime",totalTime);
        return "list/valve";
    }
    ...
    public String kikiSearchForSeikak(@ModelAttribute("KikiForm")KikiForm kikiForm, ModelMap modelMap,HttpSession session,HttpServletRequest request) throws IOException {
        double begintime=System.currentTimeMillis();
        //session　check
        User user=(User)session.getAttribute("user");
        if(user == null){
...
//            System.out.println("valve ID="+valve.getKikiSysId());
        ...
        //historyテーブルを更新
        historyValveService.addHistoryValve("",kiki.getBikou(),Config.TKikisysSearch,session,request);
        double totalTime=(System.currentTimeMillis()-begintime)/1000;
        session.setAttribute("searchValveKikiTotalTime",totalTime);
        return "list/valvekikiList";
    }
    ...
    public String buhinSearchForSeikak(@ModelAttribute("BuhinForm")BuhinForm buhinForm, ModelMap modelMap,HttpSession session,HttpServletRequest request) throws IOException {
        double begintime=System.currentTimeMillis();
        //session　check
        User user=(User)session.getAttribute("user");
        if(user == null){
        ...
        session.setAttribute("saveSelectedForBuhin","");
        //historyテーブルを更新
        historyValveService.addHistoryValve("",buhin.getBikou(),Config.TKikisysSearch,session,request);
        double totalTime=(System.currentTimeMillis()-begintime)/1000;
        session.setAttribute("searchValveBuhinTotalTime",totalTime);
        return "list/valvebuhinList";
    }
    ...
    public String valveMultSearchForSeikak(@ModelAttribute("ValveMultForm")ValveMultForm valveMultForm, ModelMap modelMap,HttpSession session,HttpServletRequest request) throws IOException {
        //session　check
        User user=(User)session.getAttribute("user");
        if(user == null){
            modelMap.addAttribute("message",Config.TUserNull);
        ...
        //全部会社名の場合は、全体検索になります
        if("全部会社名".equals(valve.getLocationName())){
            valve.setLocationName("");
            kiki.setBikou("");
            buhin.setBikou("");
        }
        ...
        List<Valve> valveResults = new ArrayList<Valve>();
        List<Valve> valveResultsLast = new ArrayList<Valve>();
        List<Kiki> kikiResults=new ArrayList<Kiki>();
        List<Buhin> buhinResults=new ArrayList<Buhin>();
        double valuebegintime=System.currentTimeMillis();
        System.out.println("valvebegintime   ="+(valuebegintime-begintime));
        valveResults=itemService.getItemByValve(valve);
        double valvetime=System.currentTimeMillis();
        System.out.println("valvetime   ="+(valvetime-valuebegintime));
        ...
        if(kiki.getKatasikiNo()=="" && kiki.getMakerRyaku()=="" && kiki.getMaker()=="" && kiki.getSyukan()==""){
            ...
            kikiIdList=itemService.getKikisystemIDByKiki(kiki);//部品と機器テーブルから弁IDしか取得ないように
        }
        double kikitime=System.currentTimeMillis();
        ...
        if(buhin.getBuhinMei()=="" && buhin.getHyojunSiyou()=="" && buhin.getSunpou()=="" && buhin.getMakerRyaku()=="" && buhin.getMaker()==""){
            ...
            buhinIdList=itemService.getKikisystemIDByBuhin(buhin);//部品と機器テーブルから弁IDしか取得ないように
        }
        double buhintime=System.currentTimeMillis();
        System.out.println("buhintime   ="+(buhintime-kikitime));
//     ----------------------  部品と機器テーブルから弁IDしか取得ないように
...
//        }
        ...
        double ketugoutime=System.currentTimeMillis();
        System.out.println("ketugoutime   ="+(ketugoutime-buhintime));
        ...
        double sorttime=System.currentTimeMillis();
        System.out.println("sorttime   ="+(sorttime-ketugoutime));
        //500件データを取り出す

        if(lastResults.size()>500){
            for(int i=0;i<500;i++){
                valveResultsLast.add(lastResults.get(i));
            }
        }else{
            valveResultsLast=lastResults;
        }
        session.setAttribute("valveMultResultsForKikisys",valveResultsLast);
        ...
        double totalTime=(System.currentTimeMillis()-begintime)/1000;
        session.setAttribute("searchValveMultiTotalTime",totalTime);
    ...
    @ResponseBody
    public String saveSelectedForValve(@RequestParam("idList")String idList,@RequestParam("type")String type,HttpSession session,ModelMap modelMap) throws UnsupportedEncodingException {
        //session　check
        User user=(User)session.getAttribute("user");
        if(user == null){
            modelMap.addAttribute("message",Config.TUserNull);
